...
# Based on matplotlib.testing.decorators.remove_ticks_and_titles,
# but customized to allow resetting the 'left' title.
def remove_text(figure):
    """Remove text (titles, tick labels) from a matplotlib figure."""
    null_formatter = NullFormatter()

    def _remove_ticks_and_title(ax):
        """Remove title and tick labels from ``ax`` and all its child Axes."""
        # Currently, all titles are 'left' titles.
        # If needed, we can also reset 'center' and 'right' titles.
        ...
        for child in ax.child_axes:
            _remove_ticks_and_title(child)

    for ax in figure.get_axes():
        _remove_ticks_and_title(ax)


def save_figure_without_text(figure, filepath):
    remove_text(figure)
    save_figure(figure, filepath)
...
def compare_snapshots(reference, actual):
    ...
    try:
        # in_decorator=True enables access to diff image for debugging.
        comparison = matplotlib.testing.compare.compare_images(
            reference, actual, 1e-6, in_decorator=True)
    except OSError:
    ...
    if comparison is None:
        return
    shutil.copy(actual, new_reference)
    # Uncomment the following lines to save the diff image for debugging.
    # diff_filename = f'{ref}_diff{ext}'
    # diff_image = comparison['diff']
    # shutil.copy(diff_image, diff_filename)
...
class RasterPlotLayoutTests(unittest.TestCase):
    ...
    def test_plot_raster_list_square_aoi(self):
        ...
        save_figure_without_text(fig, actual_png)
...
class RasterPlotConfigTests(unittest.TestCase):
